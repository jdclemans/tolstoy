...
function createBubbleChart() {
    ...
    function createNodes(rawData) {
        ...
        var myNodes = rawData.map(function (data_row) {
            node = {
                id: data_row.id,
                scaled_radius: radiusScale(+data_row[BUBBLE_PARAMETERS.radius_field]*3),
    ...
    function showLabels(mode) {
        ...
        bubble_group_labels.enter().append('text')
            ...
            .attr('y', function (d) { return mode.gridCenters[d].y - grid_element_half_height -10; })
    ...
    function tooltipContent(d) {
        ...
        for(var i=0; i<BUBBLE_PARAMETERS.tooltip.length; i++) {
            ...
            // content += '<span class="name">'  + cur_tooltip.prefix + ' </span>';
    ...
    var bubbleChart = function bubbleChart(parentDOMElement, rawData) {
        ...
        radiusScale = d3.scalePow()
            .exponent(1)
            .range([2, 40])  // Range between 2 and 25 pixels
    ...
    bubbleChart.switchMode = function (buttonID) {
        ...
        if (currentMode.type == "scatterplot") {
            xScale = d3.scaleLinear().range([0, width])
                .domain([dataExtents[currentMode.xDataField][0], dataExtents[currentMode.xDataField][1]+3]);
            yScale = d3.scaleLinear().range([height, 0])
                .domain([dataExtents[currentMode.yDataField][0], dataExtents[currentMode.yDataField][1]+280]);
